...
function (Controller, JSONModel, FlattenedDataset, FeedItem) {
    ...
    return Controller.extend("chartcolumn.controller.Main", {
        onInit: function () {
            var oData = {
                "newSAP": [
                    {"skillName": "ABAP",
        ...
        onChart: function() {
            ...
            var feedValue = new FeedItem({
                "uid": "valueAxis", // *** Chart Type에 따라 UID 다름 !!!
                "type": "Measure",
                "values": ["Rate"]
            });
    
            var feedCategory = new FeedItem({
                "uid": "categoryAxis", //
                "type": "Dimension",
                "values": ["Name"]
            });
            ...
            oVizFrame.addFeed(feedCategory);
            //
        }
    });
});
